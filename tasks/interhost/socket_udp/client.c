 ...
 * 2. bind socket fd to net params        // unnecessary for client (see comments in code)
...
int main(int argc, char* argv[])
{
   ...
   * NOTE : this step is unnecessary for UDP client.
   *        
   * COMMENT 1 : 
  ...
  inet_aton("127.0.0.1", &server.sin_addr);     // 127.0.0.1 = lockalhost = loopback adress :
                                                // IP adress, which means that server are on the same host as me
   ...
   *    ... connect(...);
   *    ... sendto(... struct sockaddr*)NULL, sizeof(server));
   * 
